...
6. 点悬浮球“搜”后：
   ...
   - 需要手动改题时，点输入框右侧的 **键盘** 按钮弹出软键盘（面板窗口本身可获焦，输入框也能直接点）
   - 面板可拖动、`—` 最小化、`×` 关闭；输入框右侧的 `问` 按钮用输入框里的文字提问
...
3. 打开本地项目文件夹（本地开发副本），把当前源码上传到仓库根目录；
   MediaProjection 修复至少涉及 `hook.py`、`android_native.py`
   和 `java/com/fojiaoai/fojiaoaisearch/ProjectionCallback.java`。
   同名文件会被覆盖；Java 文件必须保留完整目录层级
